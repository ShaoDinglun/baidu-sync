...
APP_CONFIG_TEMPLATE_PATH = CONFIG_DIR / 'config.template.json'
...
class BaiduStorage:
    ...
    def _build_simple_sync_index(self, entries):
        """基于顶层条目构建轻量同步索引。"""
        sync_index = {}
        for entry in entries:
            entry_path = self._get_list_entry_path(entry)
            entry_name = self._normalize_path(entry_path, file_only=True)
            if not entry_name:
                continue
            ...
            if isinstance(entry, dict):
                ...
                fs_id = getattr(entry, 'fs_id', None)
                size = int(getattr(entry, 'size', 0) or 0)

            sync_index[entry_name] = {
                'name': entry_name,
                'path': entry_path,
                'fs_id': str(fs_id) if fs_id is not None else '',
                'is_dir': self._is_list_entry_dir(entry),
                'mtime': self._extract_entry_mtime(entry),
                'size': size,
            }
        return sync_index

    def _list_local_file_index(self, dir_path, client=None):
        """递归列出本地目录下所有文件，返回相对路径索引。"""
        if client is None:
            client = self.client
        ...
        try:
            root_entries = self._list_dir_entries_with_fallback(dir_path, client=client)
        except Exception as e:
            if self._is_missing_path_error(e) or not self._confirm_dir_exists(dir_path, client=client, check_error=e):
                return file_index
            raise

        def _walk(current_path, entries):
            for entry in entries:
                entry_path = self._get_list_entry_path(entry)
                if not entry_path:
                    continue

                if self._is_list_entry_dir(entry):
                    try:
                        child_entries = self._list_dir_entries_with_fallback(entry_path, client=client)
                    except Exception as child_error:
                        logger.warning(f"列出本地目录失败，已跳过: {entry_path}, 错误: {str(child_error)}")
                        continue
                    _walk(entry_path, child_entries)
                elif self._is_list_entry_file(entry):
                    relative_path = posixpath.relpath(entry_path, dir_path)
                    if relative_path == '.':
                        relative_path = self._normalize_path(entry_path, file_only=True)
                    file_index[relative_path] = {
                        'path': entry_path,
                        'size': int(entry.get('size', 0) if isinstance(entry, dict) else getattr(entry, 'size', 0) or 0),
                        'mtime': self._extract_entry_mtime(entry),
                    }

        _walk(dir_path, root_entries)
        return file_index

    def _build_simple_sync_index_from_operations(self, operations):
        """基于同步操作构建轻量同步索引。"""
        sync_index = {}
        for operation in operations:
            entry = operation.get('entry')
            sync_key = operation.get('sync_key')
            if not entry or not sync_key:
                continue
            ...
            sync_index[sync_key] = {
                'name': sync_key,
                'path': self._get_list_entry_path(entry),
                'fs_id': str(fs_id) if fs_id is not None else '',
                'is_dir': self._is_list_entry_dir(entry),
                'mtime': self._extract_entry_mtime(entry),
                'size': size,
    ...
    def _get_recent_month_folder_names(self):
        """获取当前月和上一个月的目录名。"""
        current_month = datetime.now().replace(day=1)
        previous_month = (current_month - timedelta(days=1)).replace(day=1)
        return [current_month.strftime('%Y-%m'), previous_month.strftime('%Y-%m')]

    def _get_default_subscription_date_patterns(self):
        """返回订阅任务默认支持的日期目录模板。"""
        return [
            'YYYY-MM',
            'YYYY-M',
    ...
    def _normalize_subscription_month_value(self, value):
        """将月份值归一化为 YYYY-MM。"""
    ...
    def _normalize_subscription_date_patterns(self, patterns, mode='auto'):
        """标准化订阅任务日期目录模板。"""
        ...
        if mode != 'custom' or not normalized:
            return self._get_default_subscription_date_patterns()
        return normalized

    def _normalize_subscription_task_config(self, task_config):
        """标准化订阅任务同步配置，并兼容旧字段。"""
        task_config = dict(task_config or {})

        sync_mode = str(task_config.get('sync_mode') or '').strip().lower()
        if sync_mode not in {'full', 'incremental'}:
            sync_mode = 'full' if task_config.get('simple_transfer') and not task_config.get('monthly_precise_sync') else 'incremental'

        sync_scope_type = str(task_config.get('sync_scope_type') or '').strip().lower()
        if sync_scope_type not in {'all', 'recent_months', 'month_range'}:
            sync_scope_type = 'recent_months' if task_config.get('monthly_precise_sync') else 'all'

        recent_months = self._coerce_positive_int(task_config.get('recent_months'), 2)
        scope_start_month = self._normalize_subscription_month_value(task_config.get('scope_start_month'))
        scope_end_month = self._normalize_subscription_month_value(task_config.get('scope_end_month'))
        if sync_scope_type == 'month_range' and (not scope_start_month or not scope_end_month):
            sync_scope_type = 'recent_months'

        overwrite_policy = str(task_config.get('overwrite_policy') or '').strip().lower()
        if overwrite_policy not in {'never', 'window_only', 'always'}:
            if task_config.get('simple_transfer') and not task_config.get('monthly_precise_sync'):
                overwrite_policy = 'always'
            elif task_config.get('monthly_precise_sync'):
                overwrite_policy = 'window_only'
            else:
                overwrite_policy = 'never'

        date_dir_mode = str(task_config.get('date_dir_mode') or '').strip().lower()
        if date_dir_mode not in {'auto', 'custom'}:
            date_dir_mode = 'custom' if task_config.get('date_dir_patterns') else 'auto'

        date_dir_patterns = self._normalize_subscription_date_patterns(task_config.get('date_dir_patterns'), date_dir_mode)
    ...
    def _serialize_subscription_task(self, task):
        """将任务序列化为前端使用的新字段结构。"""
        serialized = dict(task or {})
        serialized.update(self._normalize_subscription_task_config(task or {}))
        return serialized

    def _migrate_subscription_tasks_in_config(self, config):
        """将配置中的旧订阅任务补齐为新的同步策略字段。"""
        ...
        for index, task in enumerate(tasks):
            ...
            normalized_task = self._serialize_subscription_task(task)
            if normalized_task != task:
                tasks[index] = normalized_task
                changed = True

        return changed

    def _build_subscription_scope_months(self, sync_config):
        """根据同步范围生成允许同步的月份集合。"""
        scope_type = sync_config.get('sync_scope_type', 'all')
        if scope_type == 'all':
            return set()

        months = set()
        if scope_type == 'recent_months':
            count = self._coerce_positive_int(sync_config.get('recent_months'), 2)
        ...
        start_month = self._normalize_subscription_month_value(sync_config.get('scope_start_month'))
        end_month = self._normalize_subscription_month_value(sync_config.get('scope_end_month'))
        if not start_month or not end_month:
            return months

        start_cursor = datetime.strptime(start_month, '%Y-%m')
        end_cursor = datetime.strptime(end_month, '%Y-%m')
    ...
    def _build_subscription_pattern_regex(self, pattern):
        """将日期目录模板转换为正则表达式。"""
        template = str(pattern or '').strip().strip('/')
        if not template:
            return None
        ...
        while index < len(template):
            if template.startswith('YYYY', index):
                regex_parts.append(r'(?P<year>\d{4})')
                index += 4
                continue
            if template.startswith('MM', index):
                regex_parts.append(r'(?P<month>0[1-9]|1[0-2])')
                index += 2
                continue
            if template.startswith('M', index):
                regex_parts.append(r'(?P<month>[1-9]|1[0-2])')
                index += 1
                continue

            regex_parts.append(re.escape(template[index]))
            index += 1

        regex_body = ''.join(regex_parts)
        return re.compile(rf'(^|.*/){regex_body}(?=/|$)')

    def _is_subscription_pattern_token_segment(self, segment):
        """判断目录模板片段是否为日期占位片段。"""
        segment_text = str(segment or '').strip()
        if not segment_text:
            return False
        return any(token in segment_text for token in ('YYYY', 'MM', 'M'))

    def _get_subscription_pattern_literal_segments(self, sync_config):
        """提取目录模板中的固定文本片段，用于递归剪枝。"""
        patterns = sync_config.get('date_dir_patterns') or self._get_default_subscription_date_patterns()
        literals = set()
        for pattern in patterns:
            for segment in str(pattern or '').strip('/').split('/'):
                segment = segment.strip()
                if segment and not self._is_subscription_pattern_token_segment(segment):
                    literals.add(segment)
        return literals

    def _extract_subscription_scope_month(self, relative_path, sync_config):
        """从相对路径中提取命中的月份值。"""
        scope_type = sync_config.get('sync_scope_type', 'all')
        if scope_type == 'all':
            return None

        normalized_path = str(relative_path or '').replace('\\', '/').strip('/').lstrip('./')
        if not normalized_path:
            return None

        patterns = sync_config.get('date_dir_patterns') or self._get_default_subscription_date_patterns()
        for pattern in patterns:
            regex = self._build_subscription_pattern_regex(pattern)
            if not regex:
                continue

            match = regex.search(normalized_path)
            if not match:
                continue
    ...
    def _is_path_in_subscription_scope(self, relative_path, sync_config, allowed_months):
        """判断路径是否命中订阅任务的时间范围。"""
        if sync_config.get('sync_scope_type') == 'all':
            return True, None

        month_key = self._extract_subscription_scope_month(relative_path, sync_config)
        if not month_key:
            return False, None
        return month_key in allowed_months, month_key

    def _should_descend_subscription_path(self, relative_path, sync_config, allowed_months):
        """判断目录路径是否仍有必要继续递归。"""
        if sync_config.get('sync_scope_type') == 'all':
            return True
        ...
        path_in_scope, matched_month = self._is_path_in_subscription_scope(normalized_path, sync_config, allowed_months)
        if path_in_scope:
            return True
        if matched_month:
            return False
        ...
        if re.fullmatch(r'\d{4}', last_segment):
            return any(month.startswith(f'{last_segment}-') for month in allowed_months)

        if len(segments) >= 2 and re.fullmatch(r'\d{4}', segments[-2]) and re.fullmatch(r'0?[1-9]|1[0-2]', last_segment):
            month_key = f"{int(segments[-2]):04d}-{int(last_segment):02d}"
            return month_key in allowed_months

        literal_segments = self._get_subscription_pattern_literal_segments(sync_config)
        for segment in segments:
            if any(literal == segment or literal in segment for literal in literal_segments):
                return True

        return False

    def _path_has_subscription_scope_hint(self, relative_path, sync_config):
        """判断路径是否包含可识别的订阅范围线索。"""
        normalized_path = str(relative_path or '').replace('\\', '/').strip('/').lstrip('./')
        if not normalized_path:
            return False

        if self._extract_subscription_scope_month(normalized_path, sync_config):
            return True
    ...
    def _should_overwrite_subscription_path(self, sync_config, path_in_scope):
        """根据覆盖策略判断是否允许覆盖已有内容。"""
        policy = sync_config.get('overwrite_policy', 'never')
        if policy == 'always':
            return True
        if policy == 'window_only':
            return sync_config.get('sync_scope_type') != 'all' and path_in_scope
        return False

    def _build_simple_transfer_operations(self, entries, save_dir, client=None, progress_callback=None, task_config=None):
        """根据条目构建简单直转的同步操作列表。"""
        if client is None:
            client = self.client

        operations = []
        month_folder_names = set(self._get_recent_month_folder_names())
        monthly_precise_sync = bool(task_config and task_config.get('monthly_precise_sync'))
        monthly_summary = {
            'archive_dirs': 0,
            'months_checked': 0,
            'files_checked': 0,
        ...
        for entry in entries:
            entry_name = self._normalize_path(self._get_list_entry_path(entry), file_only=True)
            if not entry_name:
                continue

            if monthly_precise_sync and self._is_list_entry_dir(entry) and '按月归档' in entry_name:
                monthly_summary['archive_dirs'] += 1
                archive_target_dir = self._normalize_path(posixpath.join(save_dir, entry_name))
                try:
                    month_entries = self._list_shared_paths_with_retry(
                        path=self._get_list_entry_path(entry),
                        uk=entry.uk,
                        share_id=entry.share_id,
                        bdstoken=entry.bdstoken,
                        page=1,
                        size=100,
                        client=client
                    )
                except Exception as e:
                    logger.warning(f"获取按月归档目录内容失败，已跳过: {entry_name}, 错误: {str(e)}")
                    continue

                matched_month_entries = []
                for month_entry in month_entries:
                    month_name = self._normalize_path(self._get_list_entry_path(month_entry), file_only=True)
                    if month_name in month_folder_names:
                        matched_month_entries.append(month_entry)

                if not matched_month_entries:
                    monthly_summary['months_skipped'] += len(month_folder_names)
                    logger.info(f"按月归档目录未找到当前月或上月，已跳过: {entry_name}")
                    if progress_callback:
                        progress_callback('info', f'按月归档目录未找到当前月或上月，已跳过: {entry_name}')
                    continue

                logger.info(f"按月归档目录命中 {len(matched_month_entries)} 个目标月份: {entry_name}")
                if progress_callback:
                    progress_callback('info', f'按月归档目录命中 {len(matched_month_entries)} 个目标月份: {entry_name}')

                for month_entry in matched_month_entries:
                    monthly_summary['months_checked'] += 1
                    month_name = self._normalize_path(self._get_list_entry_path(month_entry), file_only=True)
                    month_target_dir = self._normalize_path(posixpath.join(archive_target_dir, month_name))
                    local_month_index = self._list_local_file_index(month_target_dir, client=client)
                    remote_month_files = self._list_shared_dir_files(
                        month_entry,
                        month_entry.uk,
                        month_entry.share_id,
                        month_entry.bdstoken,
                        client=client
                    )

                    base_visible_path = re.sub(r'^/sharelink\d*-\d+/?', '', month_entry.path).lstrip('/')
                    missing_count = 0
                    for remote_file in remote_month_files:
                        monthly_summary['files_checked'] += 1
                        remote_path = str(remote_file.get('path', '') or '').replace('\\', '/').lstrip('/')
                        relative_path = posixpath.relpath(remote_path, base_visible_path)
                        if relative_path in ('', '.'):
                            relative_path = remote_file.get('server_filename', '')
                        relative_path = relative_path.replace('\\', '/').lstrip('./')
                        target_dir = self._normalize_path(
                            posixpath.join(month_target_dir, posixpath.dirname(relative_path) or '')
                        )
                        local_file = local_month_index.get(relative_path)
                        needs_transfer = local_file is None
                        if needs_transfer:
                            missing_count += 1
                            monthly_summary['files_changed'] += 1
                            operations.append({
                                'entry': remote_file,
                                'target_dir': target_dir,
                                'sync_key': f'{entry_name}/{month_name}/{relative_path}',
                                'use_local_compare': True,
                                'needs_transfer': True,
                            })

                    if progress_callback:
                        if missing_count > 0:
                            progress_callback('info', f'按月目录 {entry_name}/{month_name} 检测到 {missing_count} 个缺失文件')
                        else:
                            progress_callback('info', f'按月目录 {entry_name}/{month_name} 文件已齐全，已跳过')
                continue

            operations.append({
                'entry': entry,
                'target_dir': self._normalize_path(save_dir),
                'sync_key': entry_name,
            })

        if monthly_precise_sync and monthly_summary['archive_dirs'] > 0:
            summary_message = (
                f"按月精准更新汇总: 检查 {monthly_summary['archive_dirs']} 个按月归档目录，"
                f"命中 {monthly_summary['months_checked']} 个月目录，"
                f"检查 {monthly_summary['files_checked']} 个文件，"
                f"检测到 {monthly_summary['files_changed']} 个缺失文件"
            )
            logger.info(summary_message)
            if progress_callback:
                progress_callback('info', summary_message)

        return operations

    def _is_simple_sync_entry_changed(self, current_entry, cached_entry):
        """比较顶层条目快照是否发生变化。"""
        if not cached_entry:
            return True

        fields = ('fs_id', 'is_dir', 'mtime', 'size')
        for field in fields:
            if current_entry.get(field) != cached_entry.get(field):
                return True
        return False

    def _update_task_simple_sync_index(self, order, sync_index):
        """更新任务的简单直转同步索引。"""
        try:
            tasks = self.config['baidu']['tasks']
            for task in tasks:
                if task.get('order') == order:
                    task['simple_sync_index'] = sync_index
                    task['simple_sync_updated_at'] = int(time.time())
                    self._save_config()
                    logger.info(f"已更新任务简单同步索引: order={order}, items={len(sync_index)}")
                    return True
            return False
        except Exception as e:
            logger.error(f"更新任务简单同步索引失败: {str(e)}")
            return False
    ...
    def _create_config_from_template(self):
        ...
        try:
            ...
            if APP_CONFIG_PATH.exists():
                backup_path = APP_CONFIG_PATH.with_name(f'{APP_CONFIG_PATH.name}.backup.{int(time.time())}')
                shutil.copy2(APP_CONFIG_PATH, backup_path)
                logger.info(f"已备份现有配置文件到: {backup_path}")
            
            # 从模板复制配置文件
            CONFIG_DIR.mkdir(parents=True, exist_ok=True)
            shutil.copy2(template_path, APP_CONFIG_PATH)
    ...
    def add_task(self, url, save_dir, pwd=None, name=None, cron=None, category=None, regex_pattern=None, regex_replace=None, simple_transfer=False, monthly_precise_sync=False, sync_mode=None, sync_scope_type=None, recent_months=2, scope_start_month=None, scope_end_month=None, overwrite_policy=None, date_dir_mode=None, date_dir_patterns=None):
        """添加任务"""
        try:
            ...
            sync_config = self._normalize_subscription_task_config({
                'sync_mode': sync_mode,
                'sync_scope_type': sync_scope_type,
                'recent_months': recent_months,
                'scope_start_month': scope_start_month,
                ...
                'monthly_precise_sync': monthly_precise_sync,
            })
            ...
            new_task = {
                ...
                'monthly_precise_sync': False,
                'sync_mode': sync_config['sync_mode'],
                'sync_scope_type': sync_config['sync_scope_type'],
                'recent_months': sync_config['recent_months'],
                'scope_start_month': sync_config['scope_start_month'],
                'scope_end_month': sync_config['scope_end_month'],
                'overwrite_policy': sync_config['overwrite_policy'],
                'date_dir_mode': sync_config['date_dir_mode'],
                'date_dir_patterns': sync_config['date_dir_patterns'],
                'status': 'pending',
                'transferred_files': [],
                'order': new_order,
    ...
    def list_tasks(self):
        """列出所有转存任务"""
        return [self._serialize_subscription_task(task) for task in self.config['baidu']['tasks']]
    ...
    def _parse_share_error(self, error_str):
        ...
        try:
            ...
            if 'eof when reading a line' in lowered_error:
                return '分享链接当前需要验证码，服务端无法交互输入，请稍后重试或刷新账号登录状态'

            if 'error_code: -62' in error_str or "'errno': -62" in error_str:
                return '分享链接当前需要验证码，服务端无法交互输入，请稍后重试或刷新账号登录状态'
            ...
            # 检查其他常见分享错误
            if 'share' in lowered_error and 'not found' in lowered_error:
                return '分享链接不存在或已失效'
                
            if 'password' in lowered_error and 'wrong' in lowered_error:
                return '提取码错误'

            if 'vcode' in lowered_error or 'captcha' in lowered_error:
                return '分享链接当前需要验证码，服务端无法交互输入，请稍后重试或刷新账号登录状态'
    ...
    def transfer_share(self, share_url, pwd=None, new_files=None, save_dir=None, progress_callback=None, task_config=None, cancel_callback=None):
        """转存分享文件
        Args:
            share_url: 分享链接
            pwd: 提取码
            ...
            cancel_callback: 取消检查回调，返回True时终止执行
        Returns:
            dict: {
                'success': bool,  # 是否成功
                'message': str,   # 成功时的消息
        ...
        try:
            def check_cancel():
                if cancel_callback and cancel_callback():
                    logger.warning("任务已取消，终止当前转存流程")
                    if progress_callback:
                        progress_callback('warning', '任务已取消，正在停止执行')
                    return {
                        'success': False,
                        'cancelled': True,
                        'error': '任务已取消'
            ...
            try:
                ...
                normalized_task_config = self._normalize_subscription_task_config(task_config or {})
                use_new_sync_logic = bool(task_config and any(
                    key in task_config for key in (
                        'sync_mode',
                        'sync_scope_type',
                        'recent_months',
                        'scope_start_month',
                ...
                allowed_scope_months = self._build_subscription_scope_months(normalized_task_config) if use_new_sync_logic else set()

                simple_transfer = bool(task_config and task_config.get('simple_transfer')) and not use_new_sync_logic
                if simple_transfer and new_files is None:
                    logger.info("检测到直接转存模式，跳过递归扫描与增量判断")
                    if progress_callback:
                        progress_callback('info', '检测到直接转存模式，跳过递归扫描与增量判断')

                    if task_config and (task_config.get('regex_pattern') or task_config.get('regex_replace')):
                        logger.warning("直接转存模式下，文件过滤和重命名规则不会生效")
                        if progress_callback:
                            progress_callback('warning', '直接转存模式下，文件过滤和重命名规则不会生效')

                    if save_dir:
                        logger.info(f"确保直接转存目标目录存在: {save_dir}")
                        if not self._ensure_dir_tree_exists(save_dir, client=temp_client):
                            logger.error(f"创建目录失败: {save_dir}")
                            if progress_callback:
                                progress_callback('error', f'创建目录失败: {save_dir}')
                            return {'success': False, 'error': f'创建目录失败: {save_dir}'}

                    transfer_entries = list(shared_paths)
                    if len(shared_paths) == 1 and getattr(shared_paths[0], 'is_dir', False):
                        root_dir = shared_paths[0]
                        logger.info(f"直接转存模式检测到单一根目录，展开其下一层内容: {root_dir.path}")
                        if progress_callback:
                            progress_callback('info', f'检测到单一根目录，直接转存其内容: {root_dir.path}')
                        try:
                            root_children = self._list_shared_paths_with_retry(
                                path=root_dir.path,
                                uk=root_dir.uk,
                                share_id=root_dir.share_id,
                                bdstoken=root_dir.bdstoken,
                                page=1,
                                size=100,
                                client=temp_client,
                            ...
                            if root_children:
                                transfer_entries = root_children
                            else:
                                logger.warning(f"根目录为空，回退为直接转存根目录本身: {root_dir.path}")
                        except Exception as list_error:
                            logger.warning(f"获取根目录下一层内容失败，回退为直接转存根目录本身: {str(list_error)}")

                    transfer_operations = self._build_simple_transfer_operations(
                        transfer_entries,
                        save_dir,
                        client=temp_client,
                    ...
                    if cancelled_result:
                        return cancelled_result
                    current_sync_index = self._build_simple_sync_index_from_operations(transfer_operations)
                    cached_sync_index = task_config.get('simple_sync_index', {}) if task_config else {}
                    changed_operations = []
                    unchanged_operations = []
                    for operation in transfer_operations:
                        sync_key = operation.get('sync_key')
                        current_entry = current_sync_index.get(sync_key)
                        cached_entry = cached_sync_index.get(sync_key)
                        if operation.get('use_local_compare'):
                            if operation.get('needs_transfer'):
                                changed_operations.append(operation)
                            else:
                                unchanged_operations.append(operation)
                        elif self._is_simple_sync_entry_changed(current_entry, cached_entry):
                            changed_operations.append(operation)
                        else:
                            unchanged_operations.append(operation)

                    if unchanged_operations:
                        logger.info(f"简单同步命中 {len(unchanged_operations)} 个未变化项目，跳过重复转存")
                        if progress_callback:
                            progress_callback('info', f'简单同步命中 {len(unchanged_operations)} 个未变化项目，已跳过')

                    transfer_operations = changed_operations

                    if not transfer_operations:
                        logger.info("简单同步检查完成，未发现需要转存的变化项目")
                        if task_config and task_config.get('order'):
                            self._update_task_simple_sync_index(task_config.get('order'), current_sync_index)
                        return {'success': True, 'skipped': True, 'message': '没有新文件需要转存'}

                    grouped_operations = {}
                    for operation in transfer_operations:
                        grouped_operations.setdefault(operation['target_dir'], []).append(operation)

                    transferred_files = []
                    for target_dir, operations in grouped_operations.items():
                        cancelled_result = check_cancel()
                        if cancelled_result:
                            return cancelled_result

                        if not self._ensure_dir_tree_exists(target_dir, client=temp_client):
                            logger.error(f"创建目录失败: {target_dir}")
                            if progress_callback:
                                progress_callback('error', f'创建目录失败: {target_dir}')
                            return {'success': False, 'error': f'创建目录失败: {target_dir}'}

                        existing_entries = self._list_dir_entries_with_fallback(target_dir, client=temp_client)
                        existing_name_to_path = {}
                        for entry in existing_entries:
                            entry_path = self._get_list_entry_path(entry)
                            entry_name = self._normalize_path(entry_path, file_only=True)
                            if entry_name:
                                existing_name_to_path[entry_name] = entry_path

                        overwrite_paths = []
                        fs_ids = []
                        group_transferred_files = []
                        for operation in operations:
                            entry = operation['entry']
                            target_name = self._normalize_path(self._get_list_entry_path(entry), file_only=True)
                            existing_path = existing_name_to_path.get(target_name)
                            if existing_path:
                                overwrite_paths.append(existing_path)
                            fs_id = entry.get('fs_id') if isinstance(entry, dict) else getattr(entry, 'fs_id', None)
                            if fs_id is not None:
                                fs_ids.append(fs_id)
                                group_transferred_files.append(posixpath.join(target_dir, target_name))

                        if overwrite_paths:
                            logger.info(f"直接转存检测到 {len(overwrite_paths)} 个同名目标，将先删除后覆盖: {target_dir}")
                            if progress_callback:
                                progress_callback('warning', f'检测到 {len(overwrite_paths)} 个同名目标，正在删除后覆盖: {target_dir}')
                            try:
                                self._remove_remote_paths_with_retry(*overwrite_paths, client=temp_client, cancel_callback=cancel_callback)
                                logger.info(f"已删除同名目标: {overwrite_paths}")
                            except Exception as remove_error:
                                remove_msg = str(remove_error)
                                logger.error(f"删除同名目标失败: {overwrite_paths}, 错误: {remove_msg}")
                                if progress_callback:
                                    progress_callback('error', f'删除同名目标失败: {remove_msg}')
                                return {'success': False, 'error': f'删除同名目标失败: {remove_msg}'}

                        if not fs_ids:
                            continue

                        if progress_callback:
                            progress_callback('info', f'【步骤2/2】直接转存到目录: {target_dir} ({len(fs_ids)} 个项目)')

                        try:
                            logger.info(f"开始直接转存: 正在将 {len(fs_ids)} 个项目转存到 {target_dir}")
                            ...
                            logger.error(f"直接转存失败: {target_dir} - {error_msg}")
                            if progress_callback:
                                progress_callback('error', f'转存失败: {target_dir} - {error_msg}')
                            return {'success': False, 'error': f'转存失败: {target_dir} - {error_msg}'}

                        transferred_files.extend(group_transferred_files)

                    if task_config and task_config.get('order'):
                        self._update_task_simple_sync_index(task_config.get('order'), current_sync_index)
                    logger.success(f"直接转存成功，共 {len(transferred_files)} 个项目")
                    if progress_callback:
                        progress_callback('success', f'直接转存完成，共 {len(transferred_files)} 个项目')
                    return {
                        'success': True,
                        'message': f'直接转存成功，共 {len(transferred_files)} 个项目',
                ...
                for path in shared_paths:
                    ...
                    if path.is_dir:
                        ...
                        folder_files = self._list_shared_dir_files(
                            ...
                            client=temp_client,
                            cancel_callback=cancel_callback,
                            sync_config=normalized_task_config if use_new_sync_logic else None,
                            allowed_months=allowed_scope_months if use_new_sync_logic else None,
                        )
                        cancelled_result = check_cancel()
                        if cancelled_result:
                            return cancelled_result
                ...
                if use_new_sync_logic and normalized_task_config.get('sync_scope_type') != 'all':
                    detected_scope_files = 0
                    for file_info in shared_files_info:
                        if self._extract_subscription_scope_month(file_info.get('path', ''), normalized_task_config):
                            detected_scope_files += 1

                    fallback_to_full_scope = detected_scope_files == 0
                    if fallback_to_full_scope:
                        logger.info('未识别到可用的日期目录格式，本次订阅任务回退为全量范围处理')
                        if progress_callback:
                            progress_callback('warning', '未识别到可用的日期目录格式，本次回退为全量范围处理')
                ...
                local_files = []
                if save_dir:
                    local_files = self.list_local_files(save_dir, client=temp_client, cancel_callback=cancel_callback)
                    if progress_callback:
                ...
                scope_detected_files = 0
                regex_enabled = not use_new_sync_logic or normalized_task_config.get('sync_mode') != 'full'
                ...
                for file_info in shared_files_info:
                    ...
                    path_in_scope = True
                    matched_scope_month = None
                    if use_new_sync_logic:
                        path_in_scope, matched_scope_month = self._is_path_in_subscription_scope(
                            clean_path,
                            normalized_task_config,
                            allowed_scope_months
                        )
                        if matched_scope_month:
                            scope_detected_files += 1
                        if not fallback_to_full_scope and not path_in_scope:
                            continue
                        if path_in_scope:
                            scope_matched_files += 1
                    
                    # 🔄 新逻辑：先应用正则规则
                    should_transfer = True
                    final_path = clean_path
                    
                    if task_config and regex_enabled:
                    ...
                    overwrite_existing = use_new_sync_logic and self._should_overwrite_subscription_path(normalized_task_config, path_in_scope)
                    
                    if final_path != clean_path:  # 需要重命名
                        if original_exists and not final_exists and not overwrite_existing:
                            ...
                            # 添加到重命名列表（不转存）
                            rename_only_list.append({
                                'dir_path': target_dir,
                                'clean_path': clean_path,
                                'final_path': final_path,
                            })
                            continue
                        elif final_exists and not overwrite_existing:
                            # 重命名后的文件已存在
                        ...
                        if final_exists and not overwrite_existing:
                    ...
                    if new_files is None or clean_path in new_files:
                        ...
                        if target_dir is not None and clean_path is not None:
                            ...
                            transfer_list.append({
                                ...
                                'scope_month': matched_scope_month,
                            })
                ...
                if not transfer_list and not rename_only_success:
                    if progress_callback:
                        if use_new_sync_logic and normalized_task_config.get('sync_scope_type') != 'all' and not fallback_to_full_scope and scope_detected_files > 0 and scope_matched_files == 0:
                            progress_callback('warning', '未匹配到指定时间范围内的日期目录')
                        else:
                            progress_callback('info', '没有找到需要处理的文件')
                    if use_new_sync_logic and normalized_task_config.get('sync_scope_type') != 'all' and not fallback_to_full_scope and scope_detected_files > 0 and scope_matched_files == 0:
                        return {'success': True, 'skipped': True, 'message': '未匹配到指定时间范围内的日期目录'}
                    return {'success': True, 'skipped': True, 'message': '没有新文件需要转存'}
                ...
                grouped_transfers = {}
                for item in transfer_list:
                    grouped_transfers.setdefault(item['dir_path'], []).append(item)
                ...
                for dir_path, items in grouped_transfers.items():
                    cancelled_result = check_cancel()
                    if cancelled_result:
                        return cancelled_result
                    ...
                    fs_ids = [item['fs_id'] for item in items]

                    existing_entries = self._list_dir_entries_with_fallback(dir_path, client=temp_client)
                    existing_name_to_path = {}
                    for entry in existing_entries:
                        entry_path = self._get_list_entry_path(entry)
                        entry_name = self._normalize_path(entry_path, file_only=True)
                    ...
                    overwrite_paths = []
                    for item in items:
                        if not item.get('overwrite_existing'):
                            continue

                        original_name = self._normalize_path(item['clean_path'], file_only=True)
                        final_name = self._normalize_path(item['final_path'], file_only=True)
                        if original_name in existing_name_to_path:
                            overwrite_paths.append(existing_name_to_path[original_name])
                        if item.get('need_rename') and final_name in existing_name_to_path:
                            overwrite_paths.append(existing_name_to_path[final_name])

                    overwrite_paths = list(dict.fromkeys(overwrite_paths))
                    if overwrite_paths:
                        logger.info(f"检测到 {len(overwrite_paths)} 个同名目标，将先删除后覆盖: {dir_path}")
                        if progress_callback:
                            progress_callback('warning', f'检测到 {len(overwrite_paths)} 个同名目标，正在删除后覆盖: {dir_path}')
                        try:
                            self._remove_remote_paths_with_retry(*overwrite_paths, client=temp_client, cancel_callback=cancel_callback)
                        except Exception as remove_error:
                            remove_msg = str(remove_error)
                ...
                elif any(item['need_rename'] for item in transfer_list):
    ...
    def _transfer_shared_paths_with_retry(self, remotedir, fs_ids, uk, share_id, bdstoken, shared_url, client=None, cancel_callback=None):
        """带重试功能的转存方法"""
        if client is None:
            client = self.client
    ...
    @api_retry(max_retries=1, delay_range=(2, 3))
    def _access_shared_with_retry(self, share_url, pwd=None, client=None, cancel_callback=None):
        """带重试功能的访问分享链接方法"""
        if client is None:
            client = self.client
    ...
    @api_retry(max_retries=1, delay_range=(2, 3))
    def _list_shared_paths_with_retry(self, path, uk, share_id, bdstoken, page=1, size=100, client=None, cancel_callback=None):
        """带重试功能的获取分享目录内容方法"""
        if client is None:
            client = self.client
    ...
    @api_retry(max_retries=1, delay_range=(2, 3))
    def _remove_remote_paths_with_retry(self, *remotepaths, client=None, cancel_callback=None):
        """带重试功能的删除远程文件/目录方法。"""
        if client is None:
            client = self.client
        return client.remove(*remotepaths)
        ...
        try:
            ...
            for task in tasks:
                if task['url'] == task_url:
                    if status in ['running', 'success', 'skipped', 'cancelled', 'normal', 'error', 'failed']:
                        task['status'] = status
                    elif message and ('成功' in message or '没有新文件需要转存' in message):
                        task['status'] = 'normal'
                    else:
                        task['status'] = 'error'
    ...
    def list_local_files(self, dir_path, client=None, cancel_callback=None):
        """获取本地目录中的所有文件列表
        Args:
            dir_path: 目录路径
            client: 客户端实例，默认为None则使用self.client
            cancel_callback: 取消检查回调，返回True时提前停止
        """
        try:
            if client is None:
                client = self.client
            ...
            def _list_dir(path, prefetched_content=None):
                try:
                    if is_cancelled():
                        logger.warning(f"扫描本地目录时收到取消请求，停止扫描: {path}")
                        return
                    ...
                    for item in content:
                        if is_cancelled():
                            logger.warning(f"处理本地目录子项时收到取消请求，停止扫描: {path}")
                            return
        ...
        try:
            if isinstance(file_dict, dict):
                ...
                return {
                    ...
                    'mtime': self._extract_entry_mtime(file_dict),
    ...
    def _list_shared_dir_files(self, path, uk, share_id, bdstoken, client=None, cancel_callback=None, sync_config=None, allowed_months=None):
        """递归获取共享目录下的所有文件
        Args:
            path: 目录路径
            uk: 用户uk
            ...
            client: 客户端实例，默认为None则使用self.client
            cancel_callback: 取消检查回调，返回True时提前停止
            sync_config: 订阅任务同步配置，用于目录级范围剪枝
            allowed_months: 允许同步的月份集合
        Returns:
            list: 文件列表
        """
        if client is None:
        ...
        try:
            ...
            while True:
                if is_cancelled():
                    logger.warning(f"递归扫描共享目录时收到取消请求，停止扫描: {path.path}")
                    return files
            ...
            visible_dir_paths = []
            if sync_config and allowed_months is not None:
                for sub_file in sub_files:
                    if getattr(sub_file, 'is_dir', False):
                        visible_dir_paths.append(re.sub(r'^/sharelink\d*-\d+/?', '', sub_file.path).lstrip('/'))

            should_prune_by_scope = bool(
                sync_config
                and allowed_months is not None
                and any(self._path_has_subscription_scope_hint(visible_dir_path, sync_config) for visible_dir_path in visible_dir_paths)
            )
                
            for sub_file in sub_files:
                if is_cancelled():
                    logger.warning(f"处理共享目录子项时收到取消请求，停止扫描: {path.path}")
                    return files
                ...
                # 如果是目录，递归获取
                if sub_file.is_dir:
                    visible_dir_path = re.sub(r'^/sharelink\d*-\d+/?', '', sub_file.path).lstrip('/')
                    if should_prune_by_scope and not self._should_descend_subscription_path(visible_dir_path, sync_config, allowed_months):
                        logger.info(f"目录未命中订阅范围，跳过递归: {visible_dir_path}")
                        continue
    ...
    def update_task_by_order(self, order, task_data):
        ...
        try:
            ...
            sync_config = self._normalize_subscription_task_config({
                **old_task,
                'sync_mode': task_data.get('sync_mode'),
                'sync_scope_type': task_data.get('sync_scope_type'),
                'recent_months': task_data.get('recent_months', old_task.get('recent_months')),
                'scope_start_month': task_data.get('scope_start_month', old_task.get('scope_start_month')),
                'scope_end_month': task_data.get('scope_end_month', old_task.get('scope_end_month')),
                'overwrite_policy': task_data.get('overwrite_policy'),
                'date_dir_mode': task_data.get('date_dir_mode'),
                'date_dir_patterns': task_data.get('date_dir_patterns', old_task.get('date_dir_patterns')),
            })
