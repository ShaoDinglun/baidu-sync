...
from backend.notify import push_config as notify_push_config
...
APP_CONFIG_PATH = Path(__file__).resolve().parent.parent / 'config' / 'config.json'
...
class TaskScheduler:
    ...
    def get_next_run_times(self):
        ...
        for job in self.scheduler.get_jobs():
            if not str(job.id).startswith('task_'):
                continue

            task_order = None
            if job.args and isinstance(job.args[0], dict):
                task_order = job.args[0].get('order')

            if not task_order:
                match = re.match(r'^task_(\d+)(?:_\d+)?$', str(job.id))
                if not match:
                    continue
                task_order = int(match.group(1)) + 1

            if not job.next_run_time:
                next_runs[task_order] = None
                continue

            next_run = job.next_run_time.astimezone(timezone)
            current = next_runs.get(task_order)
            if current is None or next_run < current:
                next_runs[task_order] = next_run

        return {
            task_order: next_run.isoformat() if next_run else None
            for task_order, next_run in next_runs.items()
        }
    ...
    def _execute_single_task(self, task):
        ...
        Args:
            task: 任务配置
        """
        task_name = task.get('name', task.get('url', '未知任务')) if isinstance(task, dict) else '未知任务'

        # 定时任务改为等待执行锁，实现串行排队而不是跳过。
        if not self._execution_lock.acquire(blocking=False):
            logger.info(f"检测到已有任务执行中，当前定时任务进入排队等待: {task_name}")
