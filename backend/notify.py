...
def get_feishu_webhook_url() -> str:
    """获取飞书机器人 webhook 地址，兼容 FSKEY 或完整 WEBHOOK_URL 配置。"""
    fskey = str(push_config.get("FSKEY") or "").strip()
    if fskey:
        if fskey.startswith("http://") or fskey.startswith("https://"):
            return fskey
        return f'https://open.feishu.cn/open-apis/bot/v2/hook/{fskey}'

    webhook_url = str(push_config.get("WEBHOOK_URL") or "").strip()
...
def feishu_bot(title: str, content: str) -> None:
    ...
    if not url:
        print("飞书 服务的 FSKEY 或 WEBHOOK_URL 未设置!!\n取消推送")
        return
    print("飞书 服务启动")
